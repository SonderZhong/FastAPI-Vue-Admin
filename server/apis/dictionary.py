...
async def set_dictionary_list_to_cache(
    redis, cache_key: str, data: dict, expire: int = 90
):
    """设置字典列表到缓存"""
    await redis.set(cache_key, json.dumps(data, ensure_ascii=False), ex=expire)
...
async def set_dictionary_items_to_cache(
    redis, dict_id: str, data: list, expire: int = 3600
):
    """设置字典项列表到缓存"""
    cache_key = f"{RedisKeyConfig.SYSTEMDICTIONARYITEM_INFO.key}:dict:{dict_id}"
    await redis.set(cache_key, json.dumps(data, ensure_ascii=False), ex=expire)
...
async def add_dictionary(
    ...
    try:
        # 检查编码是否已存在
        existing = await SystemDictionary.get_or_none(
            dict_code=params.dict_code, is_del=False
        )
        if existing:
...
async def delete_dictionary(
    ...
    try:
        ...
        # 软删除所有字典项
        await SystemDictionaryItem.filter(dictionary_id=id, is_del=False).update(
            is_del=True
        )
...
async def delete_dictionary_list(
    ...
    try:
        ...
        for record_id in set(params.ids):
            ...
            if record:
                ...
                await SystemDictionaryItem.filter(
                    dictionary_id=record_id, is_del=False
                ).update(is_del=True)
                # 清除缓存
        ...
        return ResponseUtil.success(
            msg=f"删除成功，共删除 {deleted_count} 个数据字典！"
        )
    except Exception as e:
...
async def get_dictionary_list(
    ...
    try:
        ...
        # 尝试从缓存获取
        cached_data = await get_dictionary_list_from_cache(
            request.app.state.redis, cache_key
        )
        if cached_data:
            return ResponseUtil.success(data=cached_data)
        ...
        # 分页查询
        records = (
            await SystemDictionary.filter(**filter_args)
        ...
        # 设置缓存
        await set_dictionary_list_to_cache(
            request.app.state.redis, cache_key, response_data
        )
...
async def get_dictionary_by_code(
    ...
    try:
        ...
        # 尝试从缓存获取字典项
        cached_items = await get_dictionary_items_from_cache(
            request.app.state.redis, str(dictionary.id)
        )
        if cached_items:
        ...
        items = (
            await SystemDictionaryItem.filter(
                dictionary_id=dictionary.id, is_del=False, status=1
            )
        ...
        # 设置缓存
        await set_dictionary_items_to_cache(
            request.app.state.redis, str(dictionary.id), result
        )
...
async def add_dictionary_item(
    ...
    try:
        # 检查字典是否存在
        dictionary = await SystemDictionary.get_or_none(
            id=params.dictionary_id, is_del=False
        )
        if not dictionary:
        ...
        if result:
            # 清除字典项缓存
            await clear_dictionary_items_cache(
                request.app.state.redis, params.dictionary_id
            )
            return ResponseUtil.success(msg="添加成功！")
        else:
            return ResponseUtil.error(msg="添加失败！")
...
@Auth(
    permission_list=[
        "dictionaryitem:btn:delete",
        "DELETE,POST:/dictionary/item/delete/*",
    ]
)
...
@Auth(
    permission_list=[
        "dictionaryitem:btn:delete",
        "DELETE,POST:/dictionary/item/deleteList",
    ]
)
async def delete_dictionary_item_list(
    ...
    try:
        ...
        return ResponseUtil.success(
            msg=f"删除成功，共删除 {deleted_count} 个数据字典项！"
        )
    except Exception as e:
...
@Auth(
    permission_list=["dictionaryitem:btn:update", "PUT,POST:/dictionary/item/update/*"]
)
async def update_dictionary_item(
    ...
    try:
        ...
        if "dictionary_id" in update_data and update_data["dictionary_id"]:
            dictionary = await SystemDictionary.get_or_none(
                id=update_data["dictionary_id"], is_del=False
            )
            if not dictionary:
...
async def get_dictionary_item_info(
    ...
    """获取数据字典项信息"""
    try:
        record = await SystemDictionaryItem.get_or_none(
            id=id, is_del=False
        ...
        if record:
            data = {
                ...
                "dictionary_id": str(record.dictionary_id_id)
                if record.dictionary_id_id
                else None,
                "label": record.label,
                "value": record.value,
                "status": record.status,
...
async def get_dictionary_item_list(
    ...
    try:
        ...
        # 分页查询（预加载外键关系）
        records = (
            await SystemDictionaryItem.filter(**filter_args)
            .prefetch_related("dictionary_id")
            .offset((page - 1) * pageSize)
            .limit(pageSize)
            .all()
