...
class SystemLoginLog(DbBaseModel):
    """系统访问记录表"""

    user = fields.ForeignKeyField(
        "system.SystemUser", related_name="login_logs",
        null=True, description="用户ID", source_field="user_id",
    )
    tenant = fields.ForeignKeyField(
        "system.SystemTenant", related_name="login_logs",
        null=True, description="租户ID", source_field="tenant_id",
    )
    department = fields.ForeignKeyField(
        "system.SystemDepartment", related_name="login_logs",
        null=True, description="部门ID", source_field="department_id",
    )
    """
    用户ID。
    - 外键关联到 User 表。
...
class SystemOperationLog(DbBaseModel):
    """
    操作日志表模型。
    """
    ...
    tenant = fields.ForeignKeyField(
        "system.SystemTenant", related_name="operation_logs",
        null=True, description="租户ID", source_field="tenant_id",
    )
    department = fields.ForeignKeyField(
        "system.SystemDepartment", related_name="operation_logs",
        null=True, description="部门ID", source_field="department_id",
    )
