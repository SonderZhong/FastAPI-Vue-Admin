    ...
    async def info(self, section: Optional[str] = None) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            try:
                ...
                # 如果 psutil 不可用，使用估算值
                used_memory = len(str(self._data).encode()) + len(
                    str(self._expires).encode()
                )
            ...
            base_info = {
                ...
                "keyspace_misses": 0,
                # Keyspace 信息
                "db0": {"keys": active_keys, "expires": expires_count, "avg_ttl": 0},
            }
            ...
            if section == "server":
                ...
                return {
                    k: v
                    for k, v in base_info.items()
                    if k.startswith("used_memory")
                    or k.startswith("maxmemory")
                    or k == "mem_fragmentation_ratio"
                }
                ...
                return {
                    ...
                    for k, v in base_info.items()
                    if "client" in k
                    or k == "connected_clients"
                    or k == "blocked_clients"
                    or k == "total_connections_received"
                }
            elif section == "stats":
                return {
                    k: v
