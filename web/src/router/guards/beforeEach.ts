...
async function processBackendMenu(router: Router): Promise<void> {
  try {
    ...
    // 转换后端路由数据为前端路由格式
    const routes = (response.data as any)?.routes || response.data || []
    const menuList = convertBackendRoutesToFrontend(routes)
    await registerAndStoreMenu(router, menuList)
  } catch (error) {
 ...
 * 转换后端路由数据为前端路由格式
 */
function convertBackendRoutesToFrontend(routes: any[]): AppRouteRecord[] {
  const convertRoute = (route: any): AppRouteRecord => {
      ...
      meta: {
        ...
        authList:
          route.meta?.authList?.map((auth: any) => ({
            ...
            title: auth.authTitle || auth.title || auth.authMark
          })) || [],
        ...route.meta
      }
    }
    ...
    if (route.children && route.children.length > 0) {
      converted.children = route.children.map((child: any) => convertRoute(child))
    }

    return converted
  }

  return Array.isArray(routes) ? routes.map((route) => convertRoute(route)) : []
}
