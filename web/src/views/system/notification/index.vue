<template>
  <div class="art-full-height">
    ...
    <!-- 通知详情抽屉 -->
    <NotificationDetailDrawer v-model="detailVisible" :notification-id="currentNotification?.id" />
  </div>
</template>
...
<script setup lang="ts">
      ...
      columnsFactory: () => [
        ...
        { prop: 'title', label: '标题', align: 'center', showOverflowTooltip: true },
        {
          ...
          formatter: (row: NotificationInfo) =>
            h(ElTag, { type: getTypeTagType(row.type), size: 'small' }, () => getTypeName(row.type))
        },
        {
          prop: 'scope',
          label: '范围',
          ...
          align: 'center',
          formatter: (row: NotificationInfo) =>
            h(ElTag, { type: getScopeTagType(row.scope), size: 'small' }, () =>
        ...
        },
        {
          ...
          align: 'center',
          formatter: (row: NotificationInfo) =>
            h(ElTag, { type: getPriorityTagType(row.priority), size: 'small' }, () =>
        ...
        },
        {
          ...
          align: 'center',
          formatter: (row: NotificationInfo) =>
            h(ElTag, { type: getStatusTagType(row.status), size: 'small' }, () =>
        ...
        },
        {
          ...
          formatter: (row: NotificationInfo) => {
            ...
            buttons.push(
              h(
                ElButton,
                { type: 'primary', size: 'small', onClick: () => showDetail(row) },
                () => '详情'
              )
            )
            if (row.status === 0 && hasPermission('notification:btn:update')) {
              buttons.push(
                h(
                  ElButton,
                  { type: 'warning', size: 'small', onClick: () => showDrawer('edit', row) },
                  () => '编辑'
                )
              )
            }
            if (row.status === 0 && hasPermission('notification:btn:publish')) {
              buttons.push(
                h(
                  ElButton,
                  { type: 'success', size: 'small', onClick: () => handlePublish(row) },
                  () => '发布'
                )
              )
            }
            if (row.status === 1 && hasPermission('notification:btn:revoke')) {
              buttons.push(
                h(
                  ElButton,
                  { type: 'warning', size: 'small', onClick: () => handleRevoke(row) },
                  () => '撤回'
                )
              )
            }
  ...
  const handleRevoke = async (row: NotificationInfo) => {
    const confirm = await ElMessageBox.confirm(`确定撤回通知【${row.title}】吗？`, '确认撤回', {
      type: 'warning'
    }).catch(() => false)
    if (!confirm) return
  ...
  const handleDelete = async (row: NotificationInfo) => {
    const confirm = await ElMessageBox.confirm(`确定删除通知【${row.title}】吗？`, '确认删除', {
      type: 'warning'
    }).catch(() => false)
    if (!confirm) return
