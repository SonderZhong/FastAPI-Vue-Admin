      ...
      <div class="section">
        ...
        <ElDescriptions :column="1" border>
          ...
          <ElDescriptionsItem label="发布时间">{{
            notification.publish_time ? formatDate(notification.publish_time) : '-'
          }}</ElDescriptionsItem>
          <ElDescriptionsItem label="过期时间">{{
            notification.expire_time ? formatDate(notification.expire_time) : '永不过期'
          }}</ElDescriptionsItem>
        </ElDescriptions>
      </div>
...
<script setup lang="ts">
  ...
  const getScopeTagType = (
    ...
    const map: Record<number, 'primary' | 'warning' | 'success'> = {
      0: 'primary',
      1: 'warning',
      2: 'success'
    }
