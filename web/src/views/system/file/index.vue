<template>
  <div class="art-full-height">
    ...
    <div class="search-area">
      <div class="search-form">
        <div class="search-item">
          ...
          >
            <ElOption
              v-for="(label, key) in FileTypeLabels"
              :key="key"
              :label="label"
              :value="key"
            />
        ...
        <div class="search-item">
          ...
          >
            <ElOption
              v-for="(label, key) in StorageTypeLabels"
              :key="key"
              :label="label"
              :value="key"
            />
    ...
    <!-- 图片预览 -->
    <ElImageViewer v-if="previewVisible" :url-list="[previewUrl]" @close="previewVisible = false" />
  </div>
</template>
...
<script setup lang="ts">
      ...
      columnsFactory: () => [
        ...
        { type: 'index', width: 80, label: t('table.column.index'), align: 'center' },
        {
          ...
          formatter: (row: FileInfo) =>
            h('div', { class: 'flex items-center gap-2' }, [
              h(ElIcon, { size: 20, style: { color: getFileIconColor(row.file_type) } }, () =>
                h(getFileIconComponent(row.file_type))
              ),
              h('span', row.name)
            ])
        },
        {
          ...
          formatter: (row: FileInfo) =>
            h(
              ElTag,
              { type: getFileTypeTagType(row.file_type), size: 'small' },
        ...
        },
        {
          ...
          formatter: (row: FileInfo) => {
            ...
            buttons.push(
              h(
                ElButton,
                { type: 'primary', size: 'small', onClick: () => handlePreview(row) },
                () => t('file.preview')
              )
            )
            buttons.push(
              h(ElButton, { type: 'info', size: 'small', onClick: () => handleCopyUrl(row) }, () =>
                t('file.copyUrl')
              )
            )
  ...
  const handleBeforeUpload = (file: File) => {
    ...
    if (
      storageConfig.value.allowed_extensions.length > 0 &&
      !storageConfig.value.allowed_extensions.includes(ext)
    ) {
