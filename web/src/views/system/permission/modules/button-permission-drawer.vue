  ...
  >
    ...
    >
      ...
      <ElFormItem :label="$t('common.permissionName')" prop="name">
        ...
        <template #extra>
          <div class="mt-1 text-xs text-gray-500">{{ $t('permission.nameEnglishTip') }}</div>
        </template>
      </ElFormItem>
...
<script setup lang="ts">
  ...
  const formRules = computed(() => ({
    ...
    authMark: [
      { required: true, message: t('permission.authMarkRequired'), trigger: 'blur' },
      {
        ...
        message: t('permission.authMarkPattern'),
        trigger: 'blur'
      }
    ],
    name: [
  ...
  const drawerTitle = computed(() =>
    props.dialogType === 'add'
      ? t('permission.addButtonPermission')
      : t('permission.editButtonPermission')
  )
