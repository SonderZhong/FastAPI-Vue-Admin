<template>
  ...
  >
    ...
    <template #footer>
      ...
      <ElButton round type="primary" :loading="submitLoading" @click="handleSubmit">{{
        $t('common.confirm')
      }}</ElButton>
    </template>
  </ElDrawer>
...
<script setup lang="ts">
  ...
  const formRules = computed(() => ({
    ...
    path: [
      {
        validator: (_: any, value: string, callback: (error?: Error) => void) => {
          ...
          if (formData.value.menu_type === 0 && !formData.value.isIframe && !value)
    ...
    authTitle: [
      {
        validator: (_: any, value: string, callback: (error?: Error) => void) => {
          if (formData.value.menu_type === 1 && !value)
    ...
    api_path: [
      {
        validator: (_: any, value: string, callback: (error?: Error) => void) => {
          if (formData.value.menu_type === 2 && !value)
            callback(new Error(t('permission.apiPathRequired')))
          else callback()
        },
        trigger: 'blur'
      }
    ...
    api_method: [
      {
        validator: (_: any, value: string[], callback: (error?: Error) => void) => {
          if (formData.value.menu_type === 2 && (!value || value.length === 0))
            callback(new Error(t('permission.apiMethodRequired')))
          else callback()
        },
        trigger: 'change'
      }
    ...
    link: [
      {
        validator: (_: any, value: string, callback: (error?: Error) => void) => {
          // iframe 类型时 link 必填
          if (formData.value.menu_type === 0 && formData.value.isIframe && !value)
            callback(new Error(t('permission.linkRequired')))
          else callback()
        },
        trigger: 'blur'
      }
  ...
  const drawerTitle = computed(() =>
    props.dialogType === 'add' ? t('buttons.addPermission') : t('buttons.updatePermission')
  )
  ...
  const handleOpen = () => {
    ...
    if (props.dialogType === 'edit' && props.permissionData) {
      ...
      if (props.permissionData?.menu_type !== undefined)
        formData.value.menu_type = props.permissionData.menu_type
    }
  }
