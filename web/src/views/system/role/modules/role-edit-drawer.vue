  ...
  >
    <ElForm ref="formRef" :model="form" :rules="rules" label-width="80px" label-position="left">
      <ElFormItem :label="$t('role.roleName', '角色名称')" prop="name">
        <ElInput
          v-model="form.name"
          :placeholder="$t('common.pleaseInput', '请输入') + $t('role.roleName', '角色名称')"
        />
      </ElFormItem>
      <ElFormItem :label="$t('role.roleCode', '角色编码')" prop="code">
        <ElInput
          v-model="form.code"
          :placeholder="$t('common.pleaseInput', '请输入') + $t('role.roleCode', '角色编码')"
        />
      </ElFormItem>
      ...
      <ElFormItem :label="$t('common.description', '描述')" prop="description">
        <ElInput
          ...
          type="textarea"
          :rows="3"
          :placeholder="$t('common.pleaseInput', '请输入') + $t('common.description', '描述')"
        />
      </ElFormItem>
...
<script setup lang="ts">
  ...
  const rules = reactive<FormRules>({
    name: [
      {
        required: true,
        message: t('common.pleaseInput', '请输入') + t('role.roleName', '角色名称'),
        trigger: 'blur'
      },
    ...
    code: [
      {
        required: true,
        message: t('common.pleaseInput', '请输入') + t('role.roleCode', '角色编码'),
        trigger: 'blur'
      },
      ...
      if (response.success) {
        ElMessage.success(
          props.dialogType === 'add'
            ? t('common.addSuccess', '新增成功')
            : t('common.updateSuccess', '修改成功')
        )
        emit('success')
        handleClose()
      } else {
