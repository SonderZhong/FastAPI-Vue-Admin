...
<script setup lang="ts">
      ...
      columnsFactory: () => [
        ...
        },
        {
          ...
          formatter: (row: RoleInfo) =>
            h('span', { class: 'text-gray-500' }, row.description || '-')
        },
        {
          prop: 'status',
          ...
          formatter: (row: RoleInfo) => {
            ...
            if (hasPermission('role:btn:addPermission')) {
              buttons.push(
                h(
                  ElButton,
                  { type: 'warning', size: 'small', onClick: () => showPermissionDialog(row) },
                  () => $t('role.perm', '权限')
                )
              )
            }
            if (hasPermission('role:btn:update')) {
              buttons.push(
                h(
                  ElButton,
                  { type: 'primary', size: 'small', onClick: () => showDialog('edit', row) },
                  () => $t('buttons.edit', '编辑')
                )
              )
            }
            if (hasPermission('role:btn:delete')) {
              buttons.push(
                h(ElButton, { type: 'danger', size: 'small', onClick: () => deleteRole(row) }, () =>
                  $t('buttons.delete', '删除')
                )
              )
            }
  ...
  const deleteRole = async (role: RoleInfo) => {
    const confirm = await ElMessageBox.confirm(`确定删除角色【${role.name}】吗？`, '确认删除', {
      type: 'warning'
    }).catch(() => false)
    if (!confirm) return
