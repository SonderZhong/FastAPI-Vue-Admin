<template>
  <ElDrawer
    ...
    :title="
      dialogType === 'add'
        ? $t('department.addDepartment', '新增部门')
        : $t('department.editDepartment', '编辑部门')
    "
    size="480px"
  ...
  >
    ...
    >
      <ElFormItem :label="$t('department.name', '部门名称')" prop="name">
        <ElInput
          ...
          :placeholder="$t('common.pleaseInput', '请输入') + $t('department.name', '部门名称')"
        />
      </ElFormItem>
      ...
      <ElFormItem :label="$t('department.principal', '负责人')" prop="principal">
        <ElInput
          ...
          :placeholder="$t('common.pleaseInput', '请输入') + $t('department.principal', '负责人')"
        />
      </ElFormItem>
      ...
      <ElFormItem :label="$t('department.phone', '联系电话')" prop="phone">
        <ElInput
          ...
          :placeholder="$t('common.pleaseInput', '请输入') + $t('department.phone', '联系电话')"
        />
      </ElFormItem>
      ...
      <ElFormItem :label="$t('department.email', '邮箱')" prop="email">
        <ElInput
          ...
          :placeholder="$t('common.pleaseInput', '请输入') + $t('department.email', '邮箱')"
        />
      </ElFormItem>
      ...
      <ElFormItem :label="$t('common.remark', '备注')" prop="remark">
        <ElInput
          ...
          type="textarea"
          :placeholder="$t('common.pleaseInput', '请输入') + $t('common.remark', '备注')"
          :rows="3"
        />
      </ElFormItem>
    </ElForm>
    ...
    <template #footer>
      <div class="drawer-footer">
        ...
        <ElButton type="primary" round :loading="submitLoading" @click="handleSubmit">{{
...
<script setup lang="ts">
  ...
  const formRules: FormRules = {
    name: [
      { required: true, message: t('department.nameRequired', '请输入部门名称'), trigger: 'blur' }
    ],
    principal: [
      {
        required: true,
        message: t('department.principalRequired', '请输入负责人'),
        trigger: 'blur'
      }
    ]
  }
  ...
  const loadDepartments = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        departmentTree.value = filterTree(
          departments,
          props.dialogType === 'edit' ? props.departmentData?.id : undefined
        )
      }
    } catch (error) {
